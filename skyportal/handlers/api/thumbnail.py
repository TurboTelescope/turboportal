...
async def post_thumbnail(data, user_id, session, commit=None):
    ...
    suffix = "".join(f"_{v}" for v in (survey, phot_id) if v is not None)
    filename = f"{obj_id}_{ttype}{suffix}.png"
    ...
    if phot_id is not None:
        # Per-epoch cutouts are keyed by photometry_id, not (obj_id, type,
        # survey) -- many legitimately share obj_id/type/survey=None and must
        # stay distinct, so they bypass the survey-based upsert entirely.
        ...
        if survey is not None:
            ...
            t = await session.scalar(
                sa.select(Thumbnail).where(
                    ...
                    Thumbnail.photometry_id.is_(None),
                )
            )
            if t is None:
